...
#include "../Header Files/firstpass.h"
    ...
    while (fgets(str, MAX_LINE_LENGTH, fp) != NULL && IC + DC <= IC_MAX - IC_INIT_VALUE) {
        ...
        if (strchr(str, '.')) {
            ...
            */
            ...
            if (add_argument_machine_code_line(&code, command, &IC, 1, am_file) == 0 || \
            add_argument_machine_code_line(&code, command, &IC, 0, am_file) == 0) {
                free(command);
                error_found = 1;
                continue;
            }
