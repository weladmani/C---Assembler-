#include <stdio.h>
#include "../Header Files/secondpass.h"
...
int conv_code_octal(code_conv *code, int count, char *file_name, int IC, int DC) {
    ...
    int memorylink = 100;
    char formattedString[256]; /* Buffer to hold the formatted string */
    ...
    for (i = 0; i <= count; i++) {
        ...
        /* Format the integer and concatenate with the string */
        sprintf(formattedString, "%04d  %s", memorylink, octal_char); /* adding memory spot number format is already sorted so just adding numbers starting 0100*/
        fprintf(fp, "%s\n", formattedString);
        memorylink++;
        free(octal_char);
    }
