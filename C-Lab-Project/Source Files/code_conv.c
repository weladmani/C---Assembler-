...
#include <stdio.h>
#include <string.h>
#include "../Header Files/code_conv.h"
#include "../Header Files/Errors.h"
#include "../Header Files/globalvals.h"
...
unsigned short command_to_short(command_parts *command) {
    ...
    /* Check if the source is a register and set the corresponding bit. */
    if (what_reg(command->source) >= 0) {
        n_src = (1 << (REG_ADDRESSING + SOURCE_BITS_SHIFT));  /* lights register addressing bit. */
    }
    /* Check if the source is a register indicator and set the corresponding bit. */
    else if (what_reg_indicator(command->source) >= 0) {
        n_src = (1 << (REG_INDICATOR_ADDRESSING + SOURCE_BITS_SHIFT)); /* lights register indicator addressing bit. */
    }
        /* Check if the source is a legal label and set the corresponding bit. */
    else if (legal_label(command->source)) {
        n_src = (1 << (LABEL_ADDRESSING + SOURCE_BITS_SHIFT));  /* lights label addressing bit. */
    }
        /* Check if the source is a numerical value and set the corresponding bit. */
    else if (is_num(command->source)) {
        n_src = (1 << (DIRECT_ADDRESSING + SOURCE_BITS_SHIFT));  /* lights direct addressing bit. */
    }

    /* Check if the destination is a register and set the corresponding bit. */
    if (what_reg(command->dest) >= 0) {
        n_dest = (1 << (REG_ADDRESSING + DEST_BITS_SHIFT));  /* lights register addressing bit. */
    }
    /* Check if the destination is a register indicator and set the corresponding bit. */
    else if (what_reg_indicator(command->dest) >= 0) {
        n_dest = (1 << (REG_INDICATOR_ADDRESSING + DEST_BITS_SHIFT)); /* lights register indicator addressing bit. */
    }
        /* Check if the destination is a legal label and set the corresponding bit. */
    else if (legal_label(command->dest)) {
        n_dest = (1 << (LABEL_ADDRESSING + DEST_BITS_SHIFT));  /* lights label addressing bit. */
    }
        /* Check if the destination is a numerical value and set the corresponding bit. */
    else if (is_num(command->dest)) {
        n_dest = (1 << (DIRECT_ADDRESSING + DEST_BITS_SHIFT));  /* lights direct addressing bit. */
    }
    ...
    return ((n_src | n_op) | ( n_dest | 4)); /* lights the A bit */
}
...
unsigned short reg_to_short(command_parts *command, int reg_src) {
    ...
    if (reg_src) {
        ...
        else if ((reg1 = what_reg_indicator(command->source)) >=0) {
            n_reg_src = reg1 << SOURCE_BITS_SHIFT_REG;
        }
        ...
        else if ((reg2 = what_reg_indicator(command->dest)) >=0) {
            n_reg_dest = reg2 << DEST_BITS_SHIFT_REG;
        }
        ...
        return ((n_reg_src | n_reg_dest) | 4); /* lights the A bit */
    }
    /* Dealing with dest register, checking we didn't deal with it before we need to check if it's a register */
        ...
        return (n_reg_dest | 4); /* lights the A bit */
    }
...
int add_argument_machine_code_line(code_conv **code, command_parts *command, int *IC, int is_src, location am_file) {
    unsigned short num;
    char *arg;
     ...
     * The second condition checks if both source and dest are registers and therefore have been dealt together */
    if (what_reg(arg) >= 0 || what_reg_indicator(arg) >= 0) {
        if ((num = reg_to_short(command, is_src)) != DOUBLE_REGS_VALUE) {
            (*IC)++;
        ...
        if (legal_label(command->source) && !is_src) {
            (*IC)++;
            num = reg_to_short(command, is_src);
            if (add_machine_code_line(code, num, NULL, IC, am_file) == 0) {
                return 0;
            }
        }
    ...
    /* Check if the argument is a numerical value */
        ...
        arg++;  /* Move the pointer to skip the '#' */
        (*IC)++;
        /* representing number in 2-11 bits, therefore pushing the number ARE_BITS bits to the left adding the A bit*/
        if (add_machine_code_line(code, ((atoi(arg) << ARE_BITS) | 4), NULL, IC, am_file) == 0) {
            return 0;
        }
