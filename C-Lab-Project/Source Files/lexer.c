...
/* Define the register indicators */
char *REGSINDICATORS[] = {"*r0", "*r1", "*r2", "*r3", "*r4", "*r5", "*r6", "*r7"};
...
int what_reg_indicator(char *str) {
    int i;
    ...
    /* Iterate through the list of known registers and compare the string with each register in the list */
    for (i = 0; i < REG_COUNT; i++) {
        if (strcmp(str, REGSINDICATORS[i]) == 0) {
            return i; /* Return the index of the matching register */
        }
    }
    return -1; /* Return -1 if the string does not match any known registers */
...
int legal_label_decl(char *str, int *error_code) {
    ...
    if ((strlen(str) > MAX_LABEL_LENGTH || !isalpha(*str)) || (what_reg(str) >= 0 || what_reg_indicator(str) >= 0)) {
...
int legal_label(char *str) {
    ...
    /* If str is in one of the following - the label name is not valid */
    if (isalpha(*str) && strlen(str) <= MAX_LABEL_LENGTH && (what_reg_indicator(str) < 0) && (what_reg(str) < 0) && (what_opcode(str) < 0) && \
        !is_instr(str)) {
...
int is_reg_or_label(char *str) {
    ...
    if (what_reg_indicator(str) >= 0 || (what_reg(str) >= 0) || legal_label(str)) {
        return 1;
    }
...
int is_num(char *str) {
    ...
    if (*str == '#') {
        str++;  /* Move the pointer to skip the '#' */
    }

    /* Use strtol to check if the rest of the string is a valid number */
    strtol(str, &ptr, 10);

    /* Check if the entire string after the '#' was converted */
    if (*ptr == '\0') {
...
int legal_arg(char *str, command_parts *command, int *error_code) {
    ...
    switch (command->opcode) {
        ...
        /* source addressing code is 0,1,2,3 and dest addressing code is 0,1,2,3 (cmp) */
        case 1: {
        ...
        /* source addressing code is 0,1,2,3 and dest addressing code is 1,2,3 (mov, add, sub) */
        case 0:
        case 2:
        case 3: {
        ...
        /* source addressing code is 1 and dest addressing code is  1,2,3 (lea) */
        case 4: {
        ...
        /* dest addressing code is 1,2,3 and no source (red, dec, inc, not, clr) */
        case 5:
        case 6:
        case 7:
        case 8:
        ...
        /* dest addressing code is 1,2 and no source (jsr, bne, jmp) */
        case 9:
        case 10:
        case 13: {
            if (is_reg_or_label(str)) {
        ...
        /* dest addressing code is 0,1,2,3 and no source (prn) */
        case 12: {
...
int capture_nums(char *str, char *token_copy, inst_parts *inst, int *error_code) {
    ...
    while ((token = strtok(NULL, ", \n")) != NULL) {
        ...
        /* Convert token to a number using strtol */
        number = strtol(token, &endptr, 10);

        /* Handle different scenarios based on the token */
        if (*endptr != '\0') {
            *error_code = ERROR_CODE_50; /* Non-numeric characters found */
            return 0;
        }
        ...
        /* Allocate memory for the number array in inst_parts */
        if (inc_array(&inst, ++len) == 0) {
            return 0;
        }

        /* Store the number in the inst_parts struct */
        *(inst->nums + len - 1) = (short)number;
    }

    /* Handle case where the string ends with a comma */
    if (str[strlen(str) - 1] == ',') {
...
inst_parts *read_instruction(char *str, int *error_code) {
    ...
    else if (legal_label_decl(token, error_code)) {
        inst->label = token;
        token = strtok(NULL, " \n");
    }
