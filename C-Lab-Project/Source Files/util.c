...
char *short_to_octal(unsigned short num) {
    ...
    /* Allocate memory for the result string (5 digits + null terminator) */
    res = (char *)malloc((NUM_OCTAL_DIGITS + 1) * sizeof(char));
    if (res == NULL) {
        return  NULL;
    }

    /* Fill the string with octal digits from least significant to most significant */
    for (i = NUM_OCTAL_DIGITS - 1; i >= 0; i--) {
        res[i] = (num & OCTAL_BITS_MAX) + '0';
        num >>= OCTAL_BITS;
    }

    /* Null-terminate the string */
    res[NUM_OCTAL_DIGITS] = '\0';
